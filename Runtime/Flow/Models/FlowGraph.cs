...
namespace Ceres.Graph.Flow
{
    ...
    public class FlowGraph : CeresGraph
    {
        ...
        /// <summary>
        /// Blackboard state snapshot for hot reload
        /// </summary>
        [Serializable]
        ...
        /// <summary>
        /// Capture current blackboard state for hot reload
        /// </summary>
        /// <param name="graph"></param>
        /// <returns></returns>
        public static BlackboardState CaptureBlackboardState(FlowGraph graph)
        {
            if (graph?.Blackboard == null) return null;

            var state = new BlackboardState();
            var variableStates = new List<BlackboardState.VariableState>();

            foreach (var variable in graph.Blackboard.SharedVariables)
            {
                try
                {
                    ...
                    if (value != null)
                    {
                        // Use JsonUtility for Unity serializable types
                        if (value is UnityEngine.Object uObj)
                        {
                            // For Unity Objects, we can't serialize them directly in hot reload
                            // They will be lost, but that's acceptable for hot reload
                            continue;
                        }
                    ...
                    variableStates.Add(new BlackboardState.VariableState
                    {
                        name = variable.Name,
                        valueJson = valueJson,
            ...
            state.variables = variableStates.ToArray();
            return state;
        }

        /// <summary>
        /// Restore blackboard state from snapshot
        /// </summary>
        /// <param name="graph"></param>
        /// <param name="state"></param>
        public static void RestoreBlackboardState(FlowGraph graph, BlackboardState state)
        {
            if (graph?.Blackboard == null || state?.variables == null) return;

            foreach (var varState in state.variables)
            {
                if (string.IsNullOrEmpty(varState.name)) continue;

                // Find variable in new graph by name
                if (!graph.Blackboard.TryGetSharedVariable(varState.name, out var variable))
                {
                    // Variable doesn't exist in new graph, skip
                    continue;
                }
                ...
                {
                    // Restore value
                    if (!string.IsNullOrEmpty(varState.valueJson) && !string.IsNullOrEmpty(varState.valueType))
                    {
                        var valueType = Type.GetType(varState.valueType);
                        if (valueType != null)
                        {
                            var value = JsonUtility.FromJson(varState.valueJson, valueType);
                            variable.SetValue(value);
                        }
                    }
                    else
                    {
                        // Value is null, set to default
                        variable.SetValue(null);
                    }
                }
                catch
                {
                    // If restoration fails, continue with other variables
                    continue;
                }
            }
        }
