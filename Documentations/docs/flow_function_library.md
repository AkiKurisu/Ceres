...
`ExecutableFunctionLibrary` exposes static C# APIs to Flow through source-generated function-pointer registration.
...
The class must be `partial`; otherwise the source generator reports `Ceres001`.
...
public partial class GameplayFunctionLibrary : ExecutableFunctionLibrary
{
    ...
    public static Component Flow_GetComponent(
        GameObject target,
...
### Keep parameter count at six or fewer
...
### Reserve dependency execution for order-independent data functions

Use `ExecuteInDependency = true` only when the result depends on input values rather than forward execution order:
