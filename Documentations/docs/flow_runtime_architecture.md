...
Choose a container according to who owns the serialized graph and who owns its runtime instance.
...
Create a `FlowGraphAsset`, set its `Runtime Type` to `TestInstanceObject`, open the graph, and implement `Awake`.
