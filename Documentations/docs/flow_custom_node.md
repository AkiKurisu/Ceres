...
Create a custom node when an executable function cannot express the required execution path, state, asynchronous continuation, or port layout.
...
public class FlowNode_Delay : ForwardNode
{
    ...
    protected override async UniTask Execute(ExecutionContext executionContext)
    {
        ...
        var cancellationToken = executionContext.Context is Component component
...
- `executionContext.GetEvent()` / `GetEventT<T>()` - Get the event that triggered execution
