...
namespace Unity.Ceres.CodeGen
{
    ...
    public sealed class ExecutableReflectionILPP: ILPPInterface
    {
        ...
        {
            if (compiledAssembly.Name == CodeGenHelpers.FlowAssemblyName)
            {
                return true;
            }
            return compiledAssembly.References
                .Any(filePath => Path.GetFileNameWithoutExtension(filePath) == CodeGenHelpers.FlowAssemblyName);
        }
        ...
        public override ILPostProcessResult Process(ICompiledAssembly compiledAssembly)
        {
            ...
            {
                _diagnostics.AddError($"[{compiledAssembly.Name}] Cannot read assembly definition for Flow IL processing.");
                return null;
            }
            
            // modules
            _ceresModule = CodeGenHelpers.FindModule(assemblyDefinition, _assemblyResolver,
                CodeGenHelpers.FlowModuleName);
            
            if (_ceresModule == null)
            {
                _diagnostics.AddError($"[{compiledAssembly.Name}] Cannot find flow module: {CodeGenHelpers.FlowModuleName}");
                return null;
            }
            ...
            if (mainModule != null)
            {
                ...
                {
                _diagnostics.AddError($"[{compiledAssembly.Name}] Cannot import Flow references into module {mainModule.Name}.");
                }
            }
            else
            {
                _diagnostics.AddError($"[{compiledAssembly.Name}] Cannot get the main module for Flow IL processing.");
            }
        ...
        private void ProcessImplementableEvent(TypeDefinition typeDefinition)
        {
            ...
            if (!typeDefinition.HasInterface("Ceres.Flow.IFlowGraphRuntime"))
            {
                _diagnostics.AddWarning($"[{_mainModule.Assembly.Name.Name}] {typeDefinition.FullName}: ImplementableEvent requires IFlowGraphRuntime; ILPP skipped this type.");
                return;
            }
