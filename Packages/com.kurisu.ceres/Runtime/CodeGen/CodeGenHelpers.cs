...
namespace Unity.Ceres.CodeGen
{
    ...
    internal static class CodeGenHelpers
    {
        ...
        public const string GraphAssemblyName = "Ceres.Graph";

        public const string FlowAssemblyName = "Ceres.Flow";
        
        public const string GraphModuleName = "Ceres.Graph.dll";

        public const string FlowModuleName = "Ceres.Flow.dll";
        ...
        private static void SearchForBaseModulesRecursive(AssemblyDefinition assemblyDefinition, PostProcessorAssemblyResolver assemblyResolver, ref ModuleDefinition unityModule, ref ModuleDefinition ceresModule, HashSet<string> visited)
        {
            ...
            foreach (var module in assemblyDefinition.Modules)
            {
                ...
                if (ceresModule == null && module.Name == GraphModuleName)
                {
                    ceresModule = module;
                    continue;
                }
        ...
        public static ModuleDefinition FindModule(AssemblyDefinition assemblyDefinition,
            PostProcessorAssemblyResolver assemblyResolver, string moduleName)
        {
            var visited = new HashSet<string>();
            return FindModuleRecursive(assemblyDefinition, assemblyResolver, moduleName, visited);
        }

        private static ModuleDefinition FindModuleRecursive(AssemblyDefinition assemblyDefinition,
            PostProcessorAssemblyResolver assemblyResolver, string moduleName, HashSet<string> visited)
        {
            foreach (var module in assemblyDefinition.Modules)
            {
            ...
            {
                if (reference == null || !visited.Add(reference.Name))
                {
                    continue;
                }

                var referencedAssembly = assemblyResolver.Resolve(reference);
                if (referencedAssembly == null)
                {
                    continue;
                }

                var module = FindModuleRecursive(referencedAssembly, assemblyResolver, moduleName, visited);
                if (module != null)
                {
                    return module;
                }
