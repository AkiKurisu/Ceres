...
namespace Unity.Ceres.CodeGen
{
    internal sealed class CeresNodeILPP : ILPPInterface
    {
        ...
        {
            if (compiledAssembly.Name == CodeGenHelpers.GraphAssemblyName)
            {
                return true;
            }
            return compiledAssembly.References
                .Any(filePath => Path.GetFileNameWithoutExtension(filePath) == CodeGenHelpers.GraphAssemblyName);
        }
        ...
        public override ILPostProcessResult Process(ICompiledAssembly compiledAssembly)
        {
            ...
            {
                m_Diagnostics.AddError($"[{compiledAssembly.Name}] Cannot find graph module: {CodeGenHelpers.GraphModuleName}");
                return null;
            }
