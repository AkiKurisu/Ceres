...
public class FlowSourceGenerator
{
    protected static bool ShouldRunGenerator(GeneratorExecutionContext executionContext)
    {
        var assemblyName = executionContext.Compilation.Assembly.Name;
        return assemblyName.Equals("Ceres.Flow", StringComparison.Ordinal) ||
               assemblyName.StartsWith("Ceres.Flow.", StringComparison.Ordinal) ||
               executionContext.Compilation.ReferencedAssemblyNames.Any(r =>
                   r.Name.Equals("Ceres.Flow", StringComparison.Ordinal));
    }
